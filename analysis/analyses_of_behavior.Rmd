...
pacman::p_load(plyr,Rmisc,yarrr,BayesFactor,reshape2,brms, broom, tidyverse, brmstools, BEST, knitr, here)
# set seed
set.seed(42) 
# set directory
setwd(here())
...
```{r, warning=FALSE, message=FALSE}
summary = ddply(data.final,.(ExpPhase,Condition),plyr::summarize,Mean=c(paste(round(mean(FA.Rate, na.rm = TRUE), digits = 2), " [", round(hdi(FA.Rate)[[1]], digits = 2), " ", round(hdi(FA.Rate)[[2]], digits = 2), "]")))

names(summary) = c("Reward phase", "Reward probability", "FA Rate")
...
kable(summary, caption = "FA rates per condition")
```
...
model.full.Acc.FA = readRDS("model.full.Acc.FA.rds")
compare.waic.Acc.FA = readRDS("compare.Acc.FA.waic")
compare.Acc.FA.waic.weights = readRDS("compare.Acc.FA.waic.weights")
bR2.null.Acc.FA = readRDS("bR2.null.Acc.FA")
bR2.expphase.Acc.FA = readRDS("bR2.expphase.Acc.FA")
bR2.full.Acc.FA = readRDS("bR2.full.Acc.FA")
```

### Model comparison with WAIC

```{r, warning=FALSE, message=FALSE}
print(compare.waic.Acc.FA)
```

### Model comparison with WAIC weights

```{r, warning=FALSE, message=FALSE}
print(compare.Acc.FA.waic.weights)
```
...
Null model
```{r, warning=FALSE, message=FALSE}
print(bR2.null.Acc.FA)
```

Experiment phase model
```{r, warning=FALSE, message=FALSE}
print(bR2.expphase.Acc.FA)
```

Full model
```{r, warning=FALSE, message=FALSE}
print(bR2.full.Acc.FA)
```
...
```{r, warning=FALSE, message=FALSE}
# Plot chains
plot(model.full.Acc.FA, pars = "^b_", ask = FALSE, N=6)
```
...
```{r, warning=FALSE, message=FALSE}
# Summary of the best model
print(tidy(model.full.Acc.FA, par_type = "non-varying"), digits = 1)
```
