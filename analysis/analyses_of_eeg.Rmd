---
title: "<center> <h1>***SSVEP reward***</h1> </center>"
author: 'Ivan Grahek & Antonio Schettino'
date: '`r Sys.Date()`'
output:
  html_document:
    code_folding: hide # code is hidden but can be accessed by clicking a button
    # fig_height: 10 # height of all figures
    # fig_width: 6 # width of all figures
...
pacman::p_load(reshape2,yarrr,BayesFactor,plyr,ez,schoRsch,brms,knitr)
# set seed
set.seed(42) 
...
data$Attention = ifelse(data$AttendedColor==data$RecordedFrequency, "AttRec","NotAttRec")
...
data.diff = ddply(data, .(Subject,ExpPhase,Condition), transform, Selectivity = Amplitude[Attention=="AttRec"]-Amplitude[Attention=="NotAttRec"],TotalEnhancement=Amplitude[Attention=="AttRec"]+Amplitude[Attention=="NotAttRec"])
# Delete the Attention column and rows which are not necessary (indexes repeated twice)
data.diff = subset(data.diff,Attention=="AttRec") #keep only AttRec as it is equal to NotAttRec
...
for (i in 1:length(plottingConditions)){
  ...
  if(plottingConditions[i]=="Amplitude both reward conditions"){
    ...
    dataAmplitudePlot$Attention = factor(dataAmplitudePlot$Attention, levels = c("NotAttRec","AttRec"))
...
# model.null.threefactors = readRDS("model.null.threefactors.EEG.allsub.rds")
...
print(fixef(model.full.threefactors))

# Plot fixed effects
stanplot(model.full.threefactors, pars = "^b_")
