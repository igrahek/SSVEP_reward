...
setwd(here())
# import data
data.raw = read.csv2(file = here("data","amplitudes_rewardBoth_wholeSample_AvgRef_Movement.csv"),header=TRUE,na.strings="NaN") 
# Prepare the dataset------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

#Take 34 out to make the even number!!!!
  
data.raw = data.raw[!data.raw$Subject %in% c(34),] 

# Average the movement and no movement trials
# data.raw = ddply(data.raw,.(Subject,Frequency),transform,
#              BslnRedAttended = (BslnRedAttended + BslnRedAttendedMovement)/2,
#              BslnBlueAttended = (BslnBlueAttended + BslnBlueAttendedMovement)/2,
#              AcqRedAttended = (AcqRedAttended + AcqRedAttendedMovement)/2,
#              AcqBlueAttended = (AcqBlueAttended + AcqBlueAttendedMovement)/2,
#              ExtRedAttended = (ExtRedAttended + ExtRedAttendedMovement)/2,
#              ExtBlueAttended = (ExtBlueAttended + ExtBlueAttendedMovement)/2)

# # Select only movement or no movement trials
data.raw = select(data.raw,"Subject","Frequency","BslnRedAttended","BslnBlueAttended","AcqRedAttended","AcqBlueAttended","ExtRedAttended","ExtBlueAttended")

# Select only movement or no movement trials
# data.raw = select(data.raw,"Subject","Frequency","BslnRedAttendedMovement","BslnBlueAttendedMovement","AcqRedAttendedMovement","AcqBlueAttendedMovement","ExtRedAttendedMovement","ExtBlueAttendedMovement")
...
# Reshape to long format
# data = melt(data.raw,id.vars=c("Subject","Frequency"),
#             measure.vars=c("BslnRedAttendedMovement","BslnBlueAttendedMovement","AcqRedAttendedMovement","AcqBlueAttendedMovement","ExtRedAttendedMovement","ExtBlueAttendedMovement"),
#             variable.name="Condition",value.name="Amplitude")
