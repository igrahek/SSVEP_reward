...
%     pop_eegplot(EEG, 1, 1, 1); % check data
    ...
    for irej = 1:numel(EEG.epoch)
        trigtotepochs{irej, :} = cell2mat(EEG.epoch(1, irej).eventtype(1, 1)); % create vector with the sum of total epochs for each condition
    end
    ...
    EEG = pop_reref(EEG, [1:64], 'keepref', 'on'); % re-reference to average (the eegF_FASTER function re-references to Cz, hence the need for re-referencing here)
     ...
     EEG = pop_select(EEG, 'nochannel', [65:68]); % remove bipolar eye channels and mastoids
...
%     pop_eegplot(EEG, 1, 1, 1); % check data
    ...
    for irej2 = 1:numel(EEG.epoch)
        trigepochsleft{irej2, :} = cell2mat(EEG.epoch(1, irej2).eventtype(1, 1)); % create vector with the sum of epochs left for each condition (after artifact rejection)
    end
